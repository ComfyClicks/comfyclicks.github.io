const gallery = document.querySelector('#docketeer-gallery');
...
galleryTriggers.forEach((trigger) => trigger.addEventListener('click', () => openGallery(Number(trigger.dataset.galleryIndex), trigger)));
...
galleryStage?.addEventListener('touchstart', (event) => { touchStartX = event.touches[0].clientX; }, { passive: true });
galleryStage?.addEventListener('touchend', (event) => {
  const distance = event.changedTouches[0].clientX - touchStartX;
  if (Math.abs(distance) >= 50) showGalleryItem(currentGalleryIndex + (distance < 0 ? 1 : -1));
}, { passive: true });
